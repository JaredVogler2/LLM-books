...
OpenAI handles \textbf{text generation only} (stories, illustration prompts, character analysis). Image generation uses FAL.ai (see next section).
...
\step{7} Add \cost{\$20} in prepaid credits. This is enough to generate approximately 80--100 complete book \textit{stories} during testing (text is cheap).
...
\texttt{backend/src/ai/illustration/} & IllustrationEngineService --- generates images via Flux.2 Pro (FAL.ai) \\

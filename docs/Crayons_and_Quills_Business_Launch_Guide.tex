...
\texttt{backend/src/ai/story/} & StoryEngineService --- generates age-appropriate stories via Claude Opus 4.6 \\
...
\texttt{frontend/src/app/book/create/} & Multi-step book creation wizard (4 steps) \\
...
\subsection*{Step 3: Book Creation Wizard (4 Steps)}
The wizard (\texttt{frontend/src/app/book/create/page.tsx}) guides the customer through four steps:
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Book Options} --- Card-based selectors (not dropdowns) for:
    \begin{itemize}[leftmargin=1.5em]
      \item Story type (Bedtime, Rhyming, Adventure, Choose Your Own, STEM)
  ...
  \item \textbf{Review} --- Shows a complete summary of child details, book options, and the estimated price. The customer clicks ``Proceed to Checkout.''
\end{enumerate}
...
\begin{enumerate}[leftmargin=2em, itemsep=6pt, start=14]
  ...
  \item \textbf{Generate a test book}: On your local site, go through the book creation wizard. Enter a child's name, age, and interests. The system will:
  \begin{itemize}[leftmargin=2em]

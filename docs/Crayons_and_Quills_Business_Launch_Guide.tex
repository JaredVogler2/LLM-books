...
\section{Creating Agent Scripts}

Each agent is a simple Node.js or Python script. You do \textbf{not} need to write these yourself---use Claude Code to generate them.
...
\textit{``Create a directory called agents/ with the following agent scripts:
\begin{enumerate}[leftmargin=1.5em]
  \item order-monitor.ts --- polls my backend API every 60 seconds for new paid orders and triggers book generation via POST /api/v1/books/\{id\}/generate
  \item fulfillment-tracker.ts --- every 4 hours, checks all orders with status SENT\_TO\_PRINT and updates their fulfillment status from Lulu, sends shipping emails when status changes to shipped
  ...
  \item health-monitor.ts --- every 15 minutes, pings the backend API, Railway, OpenAI, and Stripe to verify they respond, alerts me via email if anything is down
  \item daily-digest.ts --- every day at 8 AM, compiles yesterday's order count, revenue, failed jobs, and pending issues, and emails me a summary
\end{enumerate}
...
\step{5} Claude Code will generate all six scripts. Review them, then start them with pm2:
\begin{actionbox}

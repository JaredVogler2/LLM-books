...
\hypersetup{
  ...
  bookmarks=true,
  bookmarksopen=true,
  pdftitle={StoryForge AI: Complete Business Launch Guide},
  pdfauthor={StoryForge AI},
...
\fancyhead[R]{\small\textcolor{midgray}{StoryForge AI Launch Guide}}
\fancyfoot[C]{\thepage}
...
{\fontsize{42}{50}\selectfont\bfseries\textcolor{primary}{StoryForge AI}}
...
StoryForge AI as an automated business. All pricing data is based on publicly\\
...
StoryForge AI is an online business that creates personalized children's books using artificial intelligence. A customer visits your website, enters details about their child (name, age, interests, favorite animals, etc.), and your system \textbf{automatically}:
...
AI coding tool (Bolt.new Pro) & --- & \$20 \\
...
\textbf{Monthly Operating Costs (pre-revenue)} & --- & \textbf{\$51} \\
...
\noindent At \$51/month in fixed costs, your \$1{,}314 reserve covers \textbf{25+ months} of operation even with zero revenue. You do not need to rush.
...
\begin{itemize}[leftmargin=2em]
  \item StoryForge AI LLC
  \item StoryForge Books LLC
...
\begin{itemize}[leftmargin=2em]
  \item \texttt{storyforgeai.com}
  \item \texttt{getstoryforge.com}
  \item \texttt{storyforgebooks.com}
  \item \texttt{mystoryforge.com}
\end{itemize}
...
\noindent These colors are already configured in the StoryForge codebase.
...
\midrule
9 & Bolt.new & AI app builder & \$20/mo \\
  & \multicolumn{3}{l}{\small\url{https://bolt.new} --- For building/modifying the app without coding} \\
\midrule
...
11 & Google Workspace or Gmail & Business email & Free \\
   & \multicolumn{3}{l}{\small Use a dedicated Gmail like \texttt{storyforgeai.team@gmail.com} to start} \\
\bottomrule
\end{longtable}
...
\step{8} Navigate to \textbf{API Keys}. Click \textbf{Create new secret key}. Name it ``StoryForge Production.'' \textbf{Copy this key immediately}---you will never see it again. Save it in Bitwarden.
...
GitHub account & \$0.00 \\
Bolt.new Pro (first month) & \$20.00 \\
Make.com Core (first month) & \$10.59 \\
\midrule
\textbf{Chapter 4 Total} & \textbf{\$55.59} \\
\bottomrule
\end{tabular}
\end{table}
...
% ══════════════════════════════════════════════════════════════
\chapter{Build the Application Using AI Coding Tools}

You have zero programming experience. That is fine. Modern AI coding tools can build, modify, and fix a web application for you through plain English conversation. You will not write code---you will \textbf{describe what you want} and the AI writes the code.
...
\begin{enumerate}[leftmargin=2em]
  \item Use \textbf{Bolt.new} to create a working prototype from the architecture
  \item Export the code to \textbf{GitHub}
  \item Use \textbf{Cursor} or \textbf{Windsurf} for ongoing edits and fixes
  \item Deploy to \textbf{Railway.app}
\end{enumerate}

\section{Option A: Start from the Existing Codebase (Recommended)}

Since the full codebase already exists in this repository, you can skip the Bolt.new prototyping step and deploy directly.

\step{1} Install \textbf{Cursor} on your computer:
\begin{actionbox}
\url{https://cursor.com}\\[4pt]
Download and install. It looks like a text editor (similar to Microsoft Word, but for code). Sign up for the \textbf{Hobby (free)} plan to start---this gives you a 1-week Pro trial.
\end{actionbox}

\step{2} Install \textbf{Git} (the tool that manages code versions):
\begin{actionbox}
...
\step{3} Install \textbf{Node.js} (the engine that runs the application):
\begin{actionbox}
\url{https://nodejs.org}\\[4pt]
Download the \textbf{LTS} version (not ``Current''). Run the installer. Accept all defaults.
\end{actionbox}

\step{4} Clone (download) the StoryForge codebase:
\begin{actionbox}
Open Cursor. Press \texttt{Ctrl+`} (backtick) to open the built-in terminal. Type:\\[4pt]
\texttt{git clone https://github.com/YourUsername/LLM-books.git}\\[4pt]
Then: \texttt{File} $\rightarrow$ \texttt{Open Folder} $\rightarrow$ select the \texttt{LLM-books} folder.
\end{actionbox}

\step{5} Set up the backend:
\begin{actionbox}
In Cursor's terminal, type these commands one at a time:\\[4pt]
\texttt{cd backend}\\
\texttt{npm install}\\
\texttt{cp .env.example .env}\\[4pt]
Then open the file \texttt{backend/.env} in Cursor and fill in your real API keys (OpenAI, Stripe, SendGrid, Lulu) from Chapter 4. Use the Cursor AI chat to ask: ``Help me fill in my .env file with my API keys.''
\end{actionbox}
...
\texttt{cd ../frontend}\\
\texttt{npm install}\\
\texttt{cp .env.example .env.local}\\[4pt]
Edit \texttt{frontend/.env.local} with your Stripe publishable key.
\end{actionbox}

\section{Option B: Build from Scratch with Bolt.new}

If you want to understand how the app works by building it yourself:

\step{1} Go to \url{https://bolt.new}

\step{2} In the chat, describe what you want:

\begin{actionbox}
Type a prompt like:\\[4pt]
\textit{``Build me a Next.js web application for a personalized children's book business called StoryForge AI. I need: a homepage with hero section and pricing, a 4-step book creation wizard where customers enter a child's name/age/interests and choose book options, a user dashboard showing their books and orders, Stripe payment integration, and an admin panel. Use Tailwind CSS with a purple and orange color scheme. Make it mobile-friendly.''}\\[4pt]
Bolt will generate the entire application. You can then refine by chatting: ``Make the hero section bigger,'' ``Add a gift wrap checkbox,'' etc.
\end{actionbox}

\step{3} When satisfied, click \textbf{Export to GitHub} in Bolt.new to save your code.

\section{How to Make Changes Without Coding}

Whenever you need to change something about your app, open Cursor and use the AI chat:

\begin{infobox}
\textbf{Example prompts you can type into Cursor's AI chat:}
...
\end{itemize}
Cursor reads your entire codebase and makes the changes for you.
\end{infobox}
...
\textbf{Item} & \textbf{Cost} \\
\midrule
Cursor (Hobby/free trial) & \$0.00 \\
Cursor Pro (if needed, monthly) & \$20.00 \\
Node.js & \$0.00 \\
Git & \$0.00 \\
Bolt.new Pro (optional, monthly) & \$20.00 \\
\midrule
\textbf{Chapter 5 Total} & \textbf{\$0--20.00} \\
\bottomrule
\end{tabular}
\end{table}
...
\step{9} Visit your domain in a browser. You should see your StoryForge homepage. Test:
\begin{itemize}[leftmargin=2em]
...
Cursor Pro (AI code editor) & Development & \$20.00 \\
SendGrid (free tier) & Email & \$0.00 \\
OpenAI API (testing/generation) & AI Services & \$5.00--10.00 \\
\midrule
\textbf{Monthly Total (pre-revenue)} & & \textbf{\$45.59--55.59} \\
\bottomrule
\end{tabularx}
\end{table}
...
Monthly burn rate (estimated) & \$50.00 \\
\midrule
\textbf{Months of runway (zero revenue)} & \textbf{31.7 months} \\
\bottomrule
\end{tabular}
\end{table}

\begin{successbox}
\textbf{You have nearly 32 months of runway} before you need a single sale. This means you can take your time to get the product right, test thoroughly, and build an audience before revenue pressure hits.
\end{successbox}
...
\begin{tabular}{lr}
\toprule
Monthly fixed costs & \$50.00 \\
Profit per 24-page book sale & \$18.58 \\
\midrule
\textbf{Break-even: books per month} & \textbf{2.7 $\approx$ 3 books} \\
\bottomrule
\end{tabular}
\end{table}

\begin{successbox}
\textbf{You break even at just 3 sales per month.} That is extremely achievable. At 10 sales/month, you net approximately \$136/month in profit after all costs. At 50 sales/month, approximately \$880/month.
\end{successbox}
...
Cursor Pro & \$20 & \$20 & \$20 & \$20 \\
SendGrid & \$0 & \$0 & \$20 & \$20 \\
OpenAI API & \$0 & \$25 & \$98 & \$245 \\
Domain renewal & \$1 & \$1 & \$1 & \$1 \\
\midrule
\textbf{Fixed costs} & \textbf{\$42} & \textbf{\$77} & \textbf{\$198} & \textbf{\$395} \\
\midrule
...
\textbf{Total costs} & \$42 & \$624 & \$2{,}382 & \$5{,}855 \\
\textbf{Net Profit} & \textcolor{danger}{--\$42} & \textcolor{success}{\$876} & \textcolor{success}{\$3{,}616} & \textcolor{success}{\$9{,}140} \\
\textbf{Profit Margin} & --- & 58.4\% & 60.3\% & 60.9\% \\
\bottomrule
\end{tabularx}
...
\textbf{StoryForge AI (you)} & \textbf{\$29.99--44.99} \\
\bottomrule
\end{tabularx}
\end{table}
...
Cursor & cursor.com & Account login \\
Bolt.new & bolt.new & Account login \\
...
The system that translates human-readable domain names (like \texttt{storyforge.ai}) into computer-readable IP addresses. You configure DNS records in Cloudflare to point your domain to your hosting provider.
...
A folder of code stored on GitHub. Your entire StoryForge application lives in one repository.
...
A business model where customers pay to use software online (rather than buying a physical product). Your subscription plans make StoryForge a SaaS business.
...
\textbf{Week 2} & Set up all technology accounts (Chapter 4), install Cursor, clone codebase \\
